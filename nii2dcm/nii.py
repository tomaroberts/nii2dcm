...
class Nifti:
    ...
    def get_nii2dcm_parameters(nib_nii):
        ...
        xfrm = [1, 1, 1, 1]
        axcode = nib.orientations.aff2axcodes(nib_nii.affine)
        for i, code in enumerate(axcode):
            if code not in ["R", "A", "S"]:
                xfrm[i] = -1
        A = nib_nii.affine
        dircosX = xfrm[:3] * A[:3, 0] / dimX
        dircosY = xfrm[:3] * A[:3, 1] / dimY
        ...
        nii2dcm_parameters = {
            ...
            "ImagePositionPatient": np.round(image_pos_patient_array, 10),
        }
